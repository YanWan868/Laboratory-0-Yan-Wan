...
## Other commands
Also, there are some other commands, just like: 
```bash
git diff
```
helps me quickly check exactly what lines I changed before submitting anything. Then I could use:
```bash
git add
```
to stage the files I want to include, and I can use:
```bash
git commit -m
```
to save those staged changes as a new version with a short message. Finally, I use:
```bash
git push
```
to upload my local commits to GitHub so the remote repository reflects my latest work.
